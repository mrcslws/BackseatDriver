...
class MarcusFileOutput(NuPICOutput):

  def __init__(self, *args, **kwargs):
    super(MarcusFileOutput, self).__init__(*args, **kwargs)
    self.linecount = 0
    output_filename = self.name
    print "Preparing to output to %s" % output_filename
    self.file = open(output_filename, 'w')
    self.writer = csv.writer(self.file)
    header_row = ['timestamp', 'event', 'anomalyScore', 'prediction1', 'likelihood1', 'prediction2', 'likelihood2']
    self.writer.writerow(header_row)


  def write(self, index, value, prediction_result, prediction_step=1):
    output_row = [index, value, prediction_result.inferences['anomalyScore']]
    predictions = prediction_result.inferences['multiStepPredictions'][prediction_step]
  ...
  def write_row_with_predictions(row, prediction_result):
    row.append(prediction_result.inferences['anomalyScore'])
...
class NuPICFileOutput(NuPICOutput):
  ...
  def __init__(self, *args, **kwargs):
    ...
    self.file = open(output_filename, 'w')
    self.writer = csv.writer(self.file)
    header_row = ['timestamp', 'event', 'prediction']
